...
void  TcpOptDialog::showEvent(QShowEvent* event)
{
    ...
    QString ad=url.host(); // these two function are really damn troublemakers!
    QString mp=url.path();
    Addr->insertItem(0,ad);Addr->setCurrentIndex(0);
    Port->setText(QString::number(url.port()));
    MntPnt->insertItem(0,mp);MntPnt->setCurrentIndex(0);
    ...
    int findid;
    findid=Addr->findText(ad);
    if (-1!=findid){
        Addr->setCurrentIndex(findid);
    }
    findid=MntPnt->findText(mp);
    if (-1!=findid){
        MntPnt->setCurrentIndex(findid);
    }
