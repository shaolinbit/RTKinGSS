 ...
 */
static inline int_least64_t minmea_rescale(struct minmea_float *f, int_least32_t new_scale)
{
    if (f->scale == 0)
        return 0;
...
static inline double minmea_tocoord(struct minmea_float *f)
{
	...
	long double d = (double)degrees + ((double)minutes) /(double) f->scale / (double)60;
    return d;
}
